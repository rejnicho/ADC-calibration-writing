...
\begin{figure}
	...
	\caption{Above is the combined distribution of the readout from all pixels on a single amplifier, R22 S00 C01, along all 234 ramp exposures taken in run electro-optical testing run 13549. Overlayed in orange is the output from a Savitzky-Golay filter with window size 65, and order 3. The deviations between the filter and the histogram directly 				demonstrates presence of differential nonlinearity in the ADC. In the upper righthand corner demonstrates a peak near the end of the distribution. These peaks are discussed in detail in the appendix.}
\end{figure}
\indent
...
\begin{figure}
	...
	\caption{The above figure demonstrates the calibrated bin widths of the the amplifier R22S00C01, near the lowest ADU region. At this zoom level, the substructure of the ADC can be seen clearly, as well as the characteristic macroscopic grouping of DNL (DNL = 1 - width), that was present in the earlier plots.}
\end{figure}
